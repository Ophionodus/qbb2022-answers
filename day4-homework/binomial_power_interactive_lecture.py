...
#    bosses = [10, 50, 100, 250, 500, 1000]
    trial = 0
    empty = (numpy.zeros((6, 10)))
            ...
            for n in probs:
                for k in range(n_iters):
                    ...
                    if correct_the_pvalues:    
                        ...
                        power = []
                        power[1] = compute_power(numpy.sum(pvals_translated_to_bools), n_iters)
                        empty = numpy.put_along_axis(empty, trial, power[1], None)
                        continue
    return(empty)
